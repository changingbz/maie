...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            outputs = model(samples, keep_rate, flag=flag)
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, keep_rate, flag


@torch.no_grad()
def evaluate(data_loader, model, device, keep_rate=None, flag=False):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            output = model(images, keep_rate, flag=flag)
...
def visualize_mask(data_loader, model, device, output_dir, n_visualization, fuse_token, keep_rate=None):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        acc1, acc5 = accuracy(output, target, topk=(1, 5))
        # 虽然定义了一个新的索引，只是浅拷贝，仍然使用同一块内存，也会相应改变
        # denormalize
