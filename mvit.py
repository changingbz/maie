    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            if tokens is not None:
                left_tokens = tokens
            if left_tokens == N - 1:
                return x, None, None, None, left_tokens, None
            assert left_tokens >= 1
            cls_attn = attn[:, :, 0, 1:]  # [B, H, N-1]
            cls_attn = cls_attn.mean(1)
            mean = attn[:, :, 1:, 1:].mean(-2)
            mean = mean.mean(1)
            cls_attn = cls_attn * mean
            _, idx = torch.sort(cls_attn, dim=-1, descending=True)  # [B, H, left_tokens]
            cidx = idx[:, :left_tokens]
            nidx = idx[:, left_tokens:]
            index = cidx.unsqueeze(-1).expand(-1, -1, C)  # [B, left_tokens, C]

            return x, index, cidx, cls_attn, left_tokens, nidx
        return x, None, None, None, left_tokens, None
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, aidx=None, xa=None):
        if keep_rate is None:
            keep_rate = self.keep_rate  # this is for inference, use the default keep rate
        B, N, C = x.shape

        tmp, index, idx, cls_attn, left_tokens, nidx = self.attn(self.norm1(x), keep_rate, tokens)
        ...
        if index is not None:
            ...
            cs, node_idx = distance.max(dim=-1)
            topk_attn = topk_attn.scatter_add(-1, node_idx.expand(B, -1), non_topk_attn)
            topk_attn = topk_attn.unsqueeze(-1).expand(B, topk_attn.shape[1], C)
            node_idx = node_idx.unsqueeze(dim=-1)
            x_others = x_others.scatter_add(-2, node_idx.expand(B, -1, C), non_topk)
        ...
        if get_idx and index is not None:
            return x, n_tokens, idx, nidx, cos
        return x, n_tokens, None, None, None
...
class MViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token, depth=i)
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False, flag=False):
        _, _, h, w = x.shape
        ...
        for i, blk in enumerate(self.blocks):
            x, left_token, aidx, nidx, cos = blk(x, keep_rate[i], tokens[i], get_idx, aidx, nidx)
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, flag=False):
        x, _, idxs, nidxs, coss = self.forward_features(x, keep_rate, tokens, get_idx, flag)
...
@torch.no_grad()
def _load_weights(model: MViT, checkpoint_path: str, prefix: str = ''):
...
def _create_mvit(variant, pretrained=False, default_cfg=None, **kwargs):
...
def deit_tiny_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_mvit('deit_tiny_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
...
def deit_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_mvit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model


# -------------------------------------------------------------
# MViT prototype models
@register_model
...
def deit_base_patch16_shrink_base(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    ...
    model_kwargs.update(kwargs)
    model = _create_mvit('deit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model


# -------------------------------------------------------------
# Some example MViT models
@register_model
...
def deit_small_patch16_272_shrink(pretrained=False, base_keep_rate=0.5, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6,
                        keep_rate=(1, 1, 1, 0.7) + (1, 1, 0.7) + (1, 1, 0.7) + (1, 1), **kwargs)
    model = _create_mvit('deit_small_patch16_272', pretrained=pretrained, **model_kwargs)
    return model
...
def deit_small_patch16_288_shrink06(pretrained=False, base_keep_rate=0.6, **kwargs):
    ...
    model = _create_mvit('deit_small_patch16_288', pretrained=pretrained, **model_kwargs)
    return model
...
def deit_small_patch16_304_shrink05(pretrained=False, base_keep_rate=0.5, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6,
                        keep_rate=(1, 1, 1, 0.5) + (1, 1, 0.5) + (1, 1, 0.5) + (1, 1), **kwargs)
    model = _create_mvit('deit_small_patch16_304', pretrained=pretrained, **model_kwargs)
    return model
...
def deit_base_patch16_384(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_mvit('deit_base_patch16_384', pretrained=pretrained, **model_kwargs)
    return model
