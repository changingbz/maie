...
def get_args_parser():
    ...
    parser.add_argument('--no-pin-mem', action='store_false', dest='pin_mem',
                        help='')
    parser.add_argument('--flops', action='store_true', help='Calculation quantity estimation')
...
def main(args):
    ...
    if args.flops:
       test = torch.randn(1, 3, args.input_size, args.input_size).cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, keep_rate, flag = train_one_epoch(
        ...
        if epoch % test_interval == 0 or epoch == args.epochs - 1:
            test_stats = evaluate(data_loader_val, model, device, keep_rate, flag)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
